...
**现象**：Kodi 扫描电影时，只能识别单个电影目录，电影集（如“哈利波特合集”）目录下的电影识别不出来。无论扫描多少次，电影数目总是不对（⚠️ 避坑警告：在这个设置上曾浪费过一整个晚上的排查时间）。

**原因**：这是因为开启了**“电影在包含单一视频的独立目录中”**（*Movies are in separate folders that match the movie title*）这个设置项。开启该设置后，Kodi 会死板地把第一层文件夹（例如“哈利波特合集”）当成一部单独的电影去数据库里查。查不到之后，它直接放弃，**不会再深入子目录**去扫描里面真正的电影文件。

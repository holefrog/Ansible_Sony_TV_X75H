...
### 1.1 Container Station 建容器

在 QNAP Container Station 中新建容器：

- 镜像：`mariadb:10.11`（LTS 版本，满足 Kodi 最低要求 10.2.5）
- 容器名：`kodi-mariadb`
- 端口映射：`3306:3306`
- 环境变量：
  ```
  ...
  MYSQL_USER=kodi
  MYSQL_PASSWORD=你的kodi密码
  ```
- 存储卷：`/share/Container/kodi-mariadb/data:/var/lib/mysql`（数据持久化）

### 1.2 关闭强制 SSL（重要）

Kodi Android 版连接 MariaDB 时不支持 SSL 握手，必须关闭强制 SSL，否则连接失败。

SSH 进入 QNAP，编辑容器配置或在容器内执行：

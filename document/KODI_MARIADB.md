...
**电影集（Collection）目录内的电影无法识别**

**现象**：Kodi 扫描电影时，只能识别单个电影目录，电影集（如“哈利波特合集”）目录下的电影识别不出来。（⚠️ 避坑警告：在这个设置上曾浪费过一整个晚上的排查时间）。

**原因**：这通常是由两个设置配合不当导致的（这二者相同重要，缺一不可）：
1. ❌ 开启了**“电影在包含单一视频的独立目录中”**（*Movies are in separate folders...*）：这会让 Kodi 死板地把第一层文件夹当成一部单独的电影去查，查不到直接放弃。
2. ❌ 未开启**“扫描包含的目录”**（*Scan recursively*）：只有开启递归扫描，Kodi 在遇到“电影集”文件夹时才不会浅尝辄止，而是会继续“钻”进更深层的子文件夹里，去寻找并刮削真正存放的视频文件。

**解决办法**：
1. 在 Kodi 界面选中对应的电影源，打开 **“更改内容 (Change content)”** 界面。
2. ❌ **关闭** 选项：“电影在包含单一视频的独立目录中”。
3. ✅ **开启** 选项：“扫描包含的目录” (*Scan recursively*)。
4. 点击确定保存，并在弹窗提示时选择 **“是”**，让系统重新刷新扫描即可。

> **Ansible 配置对应关系**：如果在代码中定义媒体源，此问题的修复意味着在 `group_vars/all.yml` 的 `kodi_paths` 列表中，包含电影集的路径必须设置 `use_folder_names: 0` 并且保持 `scan_recursive: 1`。
